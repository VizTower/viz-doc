...
Чтобы создать новый аккаунт в VIZ, нужно отправить специальную тразнакцию(account_create), которая зарегистрирует новую учетную запись. В этой транзакции нужно указать имя нового аккаунта([name](#name)), аккаунт-рекавери([recovery_account](#recovery_account)), аккаунт-рефера([referrer](#referrer)), заполнить метаданные аккаунта([json_metadata](#json_metadata)), вписать четыре публичных ключа: главный([master_authority](#authority)), активный([active_authority](#active_authority)), регулярный([regular_authority](#authority)), коммуникативный([memo](#authority)) - или заместо ключей привязать к одноименным ролям уже существующие аккаунты, но коммуникативный ключ все равно придется подписать, так как под него роли нет. Кроме того, отправитель транзакции должен заплатить за создание аккаунта сумму не меньшую, чем [указали делегаты](./witnesses.html#account_creation_fee), по умолчанию цена создания одного аккаунта равна 1 viz. Все ликвидные токены, которые заплатит регистратор будут конвертированы в Shares нового аккаунта.
...
| SHARES актив | "1.000000 SHARES"     | от 0.000001 SHARES | Количество [долевых токенов](./economy.html#shares). Строка с десятичным числом с не более чем 6 цифрами после точки и обязательной приписок SHARES через пробел. <br>Пример: "1.123456 SHARES" |
...
| Процент      | 1000                  | от -32768 до 10000 | Процент в целом числовом формате. 0.01% = 1; 1% = 100; 100% = 10000; |
...
[Формат](%E2%84%96props_desc_table): Процент
