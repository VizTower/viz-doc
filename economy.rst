...
обратно займет 28 дней (см. `Понижение доли <#shares-reduction>`__). При
этом коэффициент конвертации в одну и другую сторону равен 1:1 (1 shares
за 1 viz и наоборот).
...
Формула для вычисление итоговой суммы выплат после истечения срока
голосования:

``SUM(Wn * Pn) / SUM(Wn) * M``,

где ``SUM`` - функция математической суммы, ``Wn`` - количество shares
участника ``n``, ``Pn`` - процент, который выставил участник ``n``,
``M`` - максимальная сумма `ликвидных токенов <#viz-token>`__, которую
указал создатель заявки.
