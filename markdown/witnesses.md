...
В случае если делегат пропустит блок, ему назначается штраф(пенальти), который выражается во временном уменьшение количества голосов на [определённый делегатами процент](#witness_miss_penalty_percent)(по умолчанию равен 1%), и который будет погашен через некоторое время, которое так же [устанавливается делегатами](#witness_miss_penalty_duration)(по умолчанию 24 часа).

Если окажется так, что в итоге у делегата будет 0 голосов, то есть суммарный процент штрафа равен 100%, то делегат исключается из цепочки до тех пор, пока не будут погашены все штрафы. 

Кроме того возможен иной вариант исключения делегата из цепочки: если делегат пропустит более 200 блоков подряд, то он будет исключен из цепи до тех пор, пока вручную не исправит проблему и не истечет срок действия всех штрафов, заработанных за пропуск этих двухсот блоков.
...
[data_operations_cost_additional_bandwidth](#data_operations_cost_additional_bandwidth) - множитель для наценки количества пропускной способности, которую затратит аккаунт на отправку транзакций без ограничений по размеру (к таким транзакциям, например, относиться [custom-транзакция](./glossary.html#custom-transaction)).

[witness_miss_penalty_percent](#witness_miss_penalty_percent) - процент, на которой временно уменьшается количества голосов, отданных за делегата, в случае пропуска им одного блока.
...
Процент, на которой временно уменьшается количества голосов, отданных за делегата, в случае пропуска им одного блока. 
