...
способность зависит от количества [Shares](./economy.html#shares>)  на счету у пользователя.
...
На такие безграничные транзакции система может делать наценку на некоторый множитель ``X``([всё зависит от решения делегатов](./witnesses.html#data_operations_cost_additional_bandwidth)), тогда формула для ``Bnew`` будет иметь следующий вид:
