...
### Наценка на безграничные транзакции {#additional_bandwidth}

Многие транзакции в VIZ ограничены в количестве информации, которую можно в них уместит, например, при отправки перевода можно указать только количество токенов и заметку, размер который не должен превышать 2 Кбайт. Но есть и другой тип транзакций, например, custom-транзакция, в который основная информация может быть записана в вольной форме её отправителем, размер данных в таких транзакциях почти никак не ограничен.

На такие безграничные транзакции система может делать наценку на некоторый множетель ``X``([все зависит от решения делегатов](./witnesses.html#data_operations_cost_additional_bandwidth)), тогда формула для ``Bnew`` бедуте иметь слудующий вид:
